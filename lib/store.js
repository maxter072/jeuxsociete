...
function normalizeResults(body, config) {
  ...
  // ne réécrit jamais l'historique. Partie sans gagnant (mode « un seul
  // perdant », pilipili) : personne ne marque de points de rang, seulement
  // le point de participation.
  const hasWinner = cleaned.some((r) => r.rank === 1);
  return cleaned.map((r) => ({
    ...r,
    points: hasWinner ? pointsForRank(r.rank, config) : (config.participationPoints || 0),
  }));
}
